...
class Amber(CMakePackage, CudaPackage):
    ...
    # BOZ constant patch for gfortran10
    patch('amber20_gcc10_boz.patch', when="@20: %gcc@10.1.0:")
    ...
    # Cuda dependencies
    depends_on('cuda@:11.9.99', when='@20:+cuda')
    depends_on('cuda@:10.2.89', when='@18:19+cuda')
    ...
    def cmake_args(self):
        ...
        cmake_args = ["-DDOWNLOAD_MINICONDA=FALSE",
                      "-DBUILD_GUI=FALSE",
                      "-DINSTALL_TESTS=TRUE",
                      "-DBUILD_PYTHON=FALSE",
                      "-DBLA_VENDOR=All",
        ...
        # Amber uses non standard BOZ constants in many places
        if compiler == 'GNU' and spec.satisfies('%gcc@10.1.0:'):
            cmake_args.append('-DCMAKE_Fortran_FLAGS=-fallow-invalid-boz')
